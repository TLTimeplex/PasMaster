...
export const MasterPassword = (props: MasterPasswordProps) => {
  ...
  const submitForm = () => {
    if (selectedUserIndex === -1) {
      ...
      window.masterPassword
        .login(users[selectedUserIndex], password)
        .then((res) => {
          if (res) {
            props.onLogin();
          } else {
  ...
  return (
    <div id="MasterPassword">
      ...
      >
        <select
          id="MasterPasswordUserDropdownSelect"
          value={selectedUserIndex}
          onChange={hCUserSelect}
        >
        ...
        <input
          ...
          className={selectedUserIndex === -1 ? "" : "hidden"}
          onChange={hCUsername}
          value={username}
        ></input>
        <input
        ...
        ></input>
        <input
          id="MasterPasswordInputRepeat"
          type="password"
          placeholder={"Repeat Password"}
          className={selectedUserIndex === -1 ? "" : "hidden"}
          onChange={hCPasswordRepeat}
          value={repeatPassword}
        ></input>
        <button id="MasterPasswordSubmit" type="submit">
          {selectedUserIndex === -1 ? "Register" : "Login"}
        </button>
